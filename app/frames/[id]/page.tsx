...
import { FigmaFrame, FrameVersion } from '@/types'
...
export default function FrameDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: frameData }, { data: versionData }] = await Promise.all([
        supabase
          .from('frames')
          .select('*, page:pages(name, file:files(id, name, figma_file_key))')
          .eq('id', id)
          .single(),
        supabase
          .from('frame_versions')
  ...
  async function toggleVersionHistory() {
    ...
    const { data } = await supabase
      .from('frames')
      .update({ version_history_enabled: !frame.version_history_enabled })
      .eq('id', id)
      .select()
      .single()
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 grid grid-cols-3 gap-8">
        <div className="col-span-2">
          <div className="relative aspect-[4/3] bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            {frame.thumbnail_url ? (
              <Image
                ...
                {versions.map((v) => (
                  <div key={v.id} className="shrink-0 w-48">
                    <div className="relative aspect-[4/3] bg-gray-100 rounded-lg overflow-hidden border border-gray-200">
                      <Image
                        src={v.thumbnail_url}
                        alt={`Snapshot ${formatDate(v.snapshot_at)}`}
                        fill
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
            <div>
              <p className="text-xs text-gray-400 uppercase tracking-wide font-medium">Frame</p>
              <p className="text-sm text-gray-900 mt-0.5">{frame.name}</p>
            </div>
            {page?.name && (
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wide font-medium">Category</p>
                <p className="text-sm text-gray-900 mt-0.5">{page.name}</p>
              </div>
            )}
            {page?.file?.name && (
              <div>
                ...
                <Link
                  href={`/files/${page.file.id}?key=${page.file.figma_file_key}`}
            ...
            <div>
              ...
              <p className="text-sm text-gray-900 mt-0.5">{formatDate(frame.updated_at)}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">Save a snapshot on each sync</p>
              </div>
              <button
                onClick={toggleVersionHistory}
                disabled={togglingVersion}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${
                  frame.version_history_enabled ? 'bg-blue-600' : 'bg-gray-200'
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${
                    frame.version_history_enabled ? 'translate-x-6' : 'translate-x-1'
                  }`}
                />
              </button>
            </div>
          ...
          {frame.components.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              <div className="space-y-1.5 max-h-72 overflow-y-auto">
                {frame.components.map((c, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-blue-400 shrink-0" />
                    <span className="text-xs text-gray-700 truncate" title={c}>{c}</span>
                  </div>
                ))}
              </div>
            </div>
